...
// Libs
import { cn, formatToCurrency } from '../../libs';
// Styles
  ...
  function CurrencyInput({ classes, onChange, defaultValue, ...props }, ref) {
    ...
    return (
      ...
      >
        <span
          className={cn(
            'ml-3 z-0 cursor-text absolute pointer-events-none',
            classes?.icon,
          )}
        >
          R$
        ...
        <input
          placeholder="0,00"
          className={cn(
            'p-3 py-4 pl-10 w-full left-0 border border-zinc-300 top-0 rounded-sm text-right text-xs',
            classes?.input,
          )}
          {...props}
          value={value}
