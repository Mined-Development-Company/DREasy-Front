...
import { CurrencyInput, DateGroup, DateInput, ErrorMessage, Legend, Option, Select } from "..";
...
export function BillForm() {
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="p-2 bg-white border shadow-md tablet:p-5 rounded-xl"
    >
      <div className="gap-5 grid grid-cols-1 tablet:grid-cols-2 desktop:grid-cols-3">
        <label>
      ...
      <div className="flex flex-col mt-2">
        <label className="flex gap-2">
          <input
            type="checkbox"
            {...register('recurrentBill')}
          />

          <span className="text-green-700">Esta conta a pagar irá se repetir</span>
        </label>

        <label className="flex gap-2">
          <input
            type="checkbox"
            {...register('confirmedPayment')}
          />
      ...
      <div className="flex justify-end gap-5 mt-3 mobile:mt-6">
        <Button size="md" color="secondary" className="text-ellipsis line-clamp-1">
